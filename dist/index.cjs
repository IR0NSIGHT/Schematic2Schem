...
var connectors = {
    101: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [101]),
    102: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [102, 160]),
    160: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [102, 160]),
    85: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    113: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    188: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    189: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    190: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    191: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    192: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [
        85,
        113,
        107,
        183,
    ...
    139: __spreadArray(__spreadArray(__spreadArray([], blocks), stairs), [139]),
};
 ...
 */
var deconstructTag = function (tag) {
    if (!tag.includes("[")) {
        return { tag: tag, state: {} };
    }
    var tagParts = tag.split("[");
    var blockState = tagParts[1]
        .slice(0, -1)
        .split(",")
 ...
 */
var constructTag = function (tag) {
    var blockState = Object.keys(tag.state).map(function (blockState) {
 ...
 * @returns
 */
var getIdAndData = function (idx, id, data) {
    if (idx === null) {
        return null;
    }
...
var extractDimensions = function (nbt) {
    if (!nbt.value["Length"])
    ...
    var length = nbt.value["Length"].value;
    var height = nbt.value["Height"].value;
    var width = nbt.value["Width"].value;
 ...
 */
var getCoordinates = function (idx, _a) {
    var width = _a.width, length = _a.length;
    var x = idx % width;
    var y = Math.floor(idx / (width * length));
    var z = Math.floor(idx / width) % length;
    return { x: x, y: y, z: z };
};
/**
 ...
 */
var getBlockNorth = function (_a, _b) {
    var x = _a.x, y = _a.y, z = _a.z;
    var length = _b.length, width = _b.width;
 ...
 */
var getBlockEast = function (_a, _b) {
    var x = _a.x, y = _a.y, z = _a.z;
    var length = _b.length, width = _b.width;
 ...
 */
var getBlockSouth = function (_a, _b) {
    var x = _a.x, y = _a.y, z = _a.z;
    var length = _b.length, width = _b.width;
 ...
 */
var getBlockWest = function (_a, _b) {
    var x = _a.x, y = _a.y, z = _a.z;
    var length = _b.length, width = _b.width;
 ...
 */
var getSurrounding = function (idx, nbt) {
    var dimensions = extractDimensions(nbt);
    var coordinates = getCoordinates(idx, dimensions);
    var north = getBlockNorth(coordinates, dimensions);
    var east = getBlockEast(coordinates, dimensions);
    var south = getBlockSouth(coordinates, dimensions);
    var west = getBlockWest(coordinates, dimensions);
    var id = nbt.value["Blocks"].value;
    var data = nbt.value["Data"].value;
    return {
...
var fixConnectors = function (tag, _a, _b) {
    var id = _a.id;
    var north = _b.north, east = _b.east, south = _b.south, west = _b.west;
    if (north && connectors[id].includes(north.id)) {
        if (stairs.includes(north.id)) {
            var stairTag = deconstructTag(mapping.blocks[north.id + ":" + north.data]);
    ...
    if (east && connectors[id].includes(east.id)) {
        if (stairs.includes(east.id)) {
            var stairTag = deconstructTag(mapping.blocks[east.id + ":" + east.data]);
    ...
    if (south && connectors[id].includes(south.id)) {
        if (stairs.includes(south.id)) {
            var stairTag = deconstructTag(mapping.blocks[south.id + ":" + south.data]);
    ...
    if (west && connectors[id].includes(west.id)) {
        if (stairs.includes(west.id)) {
            var stairTag = deconstructTag(mapping.blocks[west.id + ":" + west.data]);
...
var extractPaletteAndBlockData = function (nbt, fast) {
    if (!nbt.value["Blocks"]) {
    ...
    var blocks = nbt.value["Blocks"].value;
    var data = nbt.value["Data"].value;
    ...
    var palette = {};
    var blockData = [];
    blocks.forEach(function (_, idx) {
        var idAndData = getIdAndData(idx, blocks, data);
        var tag = fast
    ...
    return { palette: palette, blockData: blockData };
};
var fixBlockState = function (block, idx, nbt) {
    if (!connectors[block.id] && !stairs.includes(block.id)) {
        return mapping.blocks[block.id + ":" + block.data];
    }
    var surrounding = getSurrounding(idx, nbt);
    var tag = deconstructTag(mapping.blocks[block.id + ":" + block.data]);
...
var convertPalette = function (palette) {
    var blockStates = Object.keys(palette).reduce(function (acc, blockState) {
...
var schematic2schem = function (file, fast) {
    if (fast === void 0) { fast = false; }
    return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, util.promisify(prismarineNbt.parse)(file).then(function (nbt) {
                    var dimensions = extractDimensions(nbt);
                    var convertedSchematic = extractPaletteAndBlockData(nbt, fast);
                    var paletteTag = convertPalette(convertedSchematic.palette);
                    var schematicTag = {
                        type: TagType.Compound,
                        name: "Schematic",
                        value: {
                    ...
                    var uncompressedSchematic = prismarineNbt.writeUncompressed(schematicTag);
